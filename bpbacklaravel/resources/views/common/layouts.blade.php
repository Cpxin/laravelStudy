        ...
        <!--右侧内容区域-->
            <div id="rightContent" style="background:url({{asset('img/中秋国庆纹理.png')}});width:100px;height:100%;">
                @if($_SERVER['REQUEST_URI']=='/bpbacklaravel/public/layouts')
                    ...
                    <img id="toout" onclick="window.location.href='{{url('admin/logout')}}'"
                         src="{{asset('img/退出.png')}}"
                         style="position:fixed;top: 20px;right:20px;height: 30px;width: 30px">
                @else
                    @yield('content')
                    <img id="toback" src="{{asset('img/后退.png')}}"
                         style="position: fixed;bottom:20px;right: 20px;height: 50px;width: 50px">
                @endif
